...
class College extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, type, province, uid', 'required'),
			array('type, city, province, uid', 'numerical', 'integerOnly'=>true),
			array('name, homepage', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, type, city, province, homepage, uid', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'profiles' => array(self::HAS_MANY, 'Profile', 'middleschool'),
			'profiles1' => array(self::HAS_MANY, 'Profile', 'highschool'),
			'profiles2' => array(self::HAS_MANY, 'Profile', 'university'),
		);
	}
